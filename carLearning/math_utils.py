...
def cross_lines(pointA, vecA, pointB, vecB):
    '''returns the intersection point between 2 lines'''
    const_A = orthogonal_dot(vecA, pointA)
    const_B = orthogonal_dot(vecB, pointB)
    x = - (vecA[0] * const_B - const_A * vecB[0]) / (- vecA[1] * vecB[0] + vecB[1] * vecA[0])
    y = - (- vecA[1] * const_B + const_A * vecB[1]) / (- vecA[0] * vecB[1] + vecB[0] * vecA[1])
    return np.array([x, y])

def modify_v(v, scaler = 0.01):
