...
for i in range(n):
    if i%1000==0 and i!=0:
        ...
        print('total',solved+unsolved)
        print('success rate:',100*(solved/solved+unsolved),'%')
        print('in:',time()-t0,'seconds, so',(solved+unsolved)/(time()-t0),'solved per second')
    M=ex.createM(np.transpose(quizzes[i]))
    unsolvedMatrixes.append(M)
    worked,M=roll(M,5)
    N=ex.fromMtoArray(M)
    solvedMatrixes.append(N)
    successBooleans.append(ex.check(N))
    if successBooleans[-1]==True:
        solved+=1
    else:
        unsolved+=1
print('----------------------------')
